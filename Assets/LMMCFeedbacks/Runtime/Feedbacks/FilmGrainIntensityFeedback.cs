...
.WithScheduler(LitMotion.Editor.EditorMotionScheduler.Update)
                ...
                .Bind(value =>
                {
                    var filmGrain = target.TryGetVolumeComponent<UnityEngine.Rendering.Universal.FilmGrain>();
                    filmGrain.EnableVolumeparameterAll();
                    filmGrain.intensity.Override(value);
                });
            return Handle;
        }
